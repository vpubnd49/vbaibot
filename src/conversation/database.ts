...
function runMigrations(): void {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_fin_tx_account_thread ON financial_transactions (account_id, thread_id, transaction_date);
    ...
    CREATE TABLE IF NOT EXISTS financial_categories (
      ...
      thread_id TEXT NOT NULL,
      name TEXT NOT NULL,
      type TEXT NOT NULL CHECK (type IN ('thu','chi','both')),
      icon TEXT NOT NULL DEFAULT '📋',
      UNIQUE (account_id, thread_id, name)
    );
    ...
    CREATE TABLE IF NOT EXISTS shared_knowledge (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      account_id TEXT NOT NULL,
      category TEXT NOT NULL CHECK (category IN ('legal','policy','procedure','correction','general')),
      content TEXT NOT NULL,
      source TEXT NOT NULL DEFAULT '',
      status TEXT NOT NULL CHECK (status IN ('pending','approved','rejected')) DEFAULT 'pending',
      reviewed_by TEXT,
      learned_in_thread_id TEXT NOT NULL DEFAULT '',
      created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
      approved_at TEXT,
      expires_at TEXT
    );
    CREATE INDEX IF NOT EXISTS idx_shared_knowledge_account_status
      ON shared_knowledge (account_id, status, id);
  ...
  // File tài liệu kèm tin nhắn: JSON array các file { fileName, localPath, extension }
  addColumnIfMissing("messages", "files", "TEXT");
