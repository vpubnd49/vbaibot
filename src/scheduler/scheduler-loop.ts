...
export function startScheduler(): void {
  ...
  // Đồng bộ lịch chào buổi sáng 6:30 cho tất cả các nhóm (hoặc xóa nếu đang tắt)
  import("./morning-greeting.js")
    .then(({ syncAllAccountsMorningGreeting, isMorningGreetingEnabled }) => {
      if (isMorningGreetingEnabled()) {
        syncAllAccountsMorningGreeting();
      } else {
        // Tính năng đang tắt — xóa hết job chào sáng cũ còn sót
        import("./morning-greeting.js").then(({ removeMorningGreetingJobs }) => {
          import("../config/account-store.js").then(({ listEnabledAccounts }) => {
            for (const acc of listEnabledAccounts()) removeMorningGreetingJobs(acc.id);
          });
        });
      }
    })
    .catch((err) => log.error({ err }, "Lỗi khi đồng bộ job chào buổi sáng"));

  // Đồng bộ các bản tin thời sự ngẫu nhiên (sáng/chiều/tối) cho tất cả các nhóm (hoặc xóa nếu đang tắt)
  import("./random-news-broadcast.js")
    .then(({ syncAllAccountsRandomNews, isRandomNewsBroadcastEnabled }) => {
      if (isRandomNewsBroadcastEnabled()) {
        syncAllAccountsRandomNews();
      } else {
        // Tính năng đang tắt — xóa hết job bản tin cũ còn sót
        import("./random-news-broadcast.js").then(({ removeAllRandomNewsJobs }) => {
          import("../config/account-store.js").then(({ listEnabledAccounts }) => {
            for (const acc of listEnabledAccounts()) removeAllRandomNewsJobs(acc.id);
          });
        });
      }
    })
    .catch((err) => log.error({ err }, "Lỗi khi đồng bộ job bản tin thời sự ngẫu nhiên"));
