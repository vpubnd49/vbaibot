...
describe("đánh dấu người ngoài allowlist", () => {
  ...
  it("ngân sách tài liệu history ưu tiên tin mới nhất, tin cũ rơi về text", () => {
    const history: StoredMessage[] = [
      {
        role: "user",
        content: "file 1 [gửi kèm 1 file: doc1.docx]",
        senderName: "Hải",
        files: [{ fileName: "doc1.docx", localPath: "media/a/t/doc1.docx", extension: ".docx" }],
      },
      {
        role: "user",
        content: "file 2 [gửi kèm 1 file: doc2.docx]",
        senderName: "Hải",
        files: [{ fileName: "doc2.docx", localPath: "media/a/t/doc2.docx", extension: ".docx" }],
      },
    ];

    const plan = planDocumentBudget(history, 1);
    assert.equal(plan.size, 1);
    assert.equal(plan.has(1), true);

    const paths = collectDocumentPathsWithinBudget(history, 1);
    assert.deepEqual(paths, ["media/a/t/doc2.docx"]);

    const fakeDocLoader = (relPath: string) => ({
      text: `Nội dung của ${relPath}`,
      truncated: false,
    });
    ...
    // Tin 0 (cũ hơn) không nạp nội dung file vì docLimit = 1
    assert.equal(typeof out[0]!.content, "string");
    assert.match(String(out[0]!.content), /doc1\.docx/);

    // Tin 1 (mới nhất) có nạp nội dung file
    const parts = out[1]!.content as { type: string; text?: string }[];
    assert.ok(Array.isArray(parts));
    const docPart = parts.find((p) => p.text?.includes("Nội dung của media/a/t/doc2.docx"));
    assert.ok(docPart);
  });
});
