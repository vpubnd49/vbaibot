...
const envSchema = z.object({
  ...
  // Text-to-Speech (voice podcast). Dùng Gemini Flash TTS, tính theo token.
  // Nếu LLM_PROVIDER=google thì TTS tái sử dụng LLM_API_KEY, không cần set riêng.
  TTS_API_KEY: z.string().default(""),
  TTS_MODEL: z.string().default("gemini-2.5-flash-preview-tts"),
  // URL công khai để Zalo server tải file audio (VD: https://bot.lamdong.vn)
  TTS_PUBLIC_BASE_URL: z.string().default(""),
  // Giọng đọc Gemini (xem danh sách tại https://ai.google.dev/gemini-api/docs/text-to-speech)
  TTS_MALE_VOICE: z.string().default("Orus"),
  TTS_FEMALE_VOICE: z.string().default("Aoede"),
