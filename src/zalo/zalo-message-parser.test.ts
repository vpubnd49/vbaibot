...
describe("parseIncomingMessage", () => {
  ...
  it("trích xuất file khi content là chuỗi JSON mã hóa", () => {
    const jsonContent = JSON.stringify({
      title: "BC tuần 33.docx",
      href: "https://files.zalo.me/bc33.docx",
      size: 102400,
    });
    const msg = parseIncomingMessage("acc-test", SELF_ID, {
      threadId: "group-1",
    ...
    assert.equal(msg.files[0]!.fileName, "BC tuần 33.docx");
    assert.equal(msg.files[0]!.url, "https://files.zalo.me/bc33.docx");
  });
});
