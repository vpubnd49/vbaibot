...
function blockToElements(block: DocumentBlock): (Paragraph | Table)[] {
  switch (block.type) {
    ...
    case "two_columns":
      return [
        buildTwoColumns(block.left, block.right, {
          left_align: block.left_align,
          right_align: block.right_align,
          ratio: block.ratio as [number, number] | undefined,
        }),
        new Paragraph({ text: "" }),
      ];

    case "separator":
      return [buildSeparator(block.width_percent)];
  }
}

/**
 * Đường kẻ ngang trang trí theo NĐ 30:
 * - Dưới tên cơ quan: ~33% bề rộng, canh giữa
 * - Dưới tiêu ngữ: ~100% bề rộng dòng chữ, canh giữa
 */
function buildSeparator(widthPercent: number): Paragraph {
  const dashCount = Math.max(3, Math.round(widthPercent / 3));
...
export async function renderDocx(blocks: DocumentBlock[], meta: DocxMeta = {}): Promise<Buffer> {
  ...
  // Đánh số trang ở chân trang, canh giữa, 13pt - theo NĐ 30
