...
import {
  collectDocumentPathsWithinBudget,
...
import { loadUploadedDocument } from "../documents/load-uploaded-document.js";
...
export async function buildTurnMessages(params: {
  ...
  const docLimit = getTuning("DOCUMENT_READ_HISTORY_LIMIT");
  const docMaxChars = getTuning("DOCUMENT_READ_MAX_CHARS");
  const docCache = new Map<string, Awaited<ReturnType<typeof loadUploadedDocument>>>();

  if (docLimit > 0) {
    const docPaths = collectDocumentPathsWithinBudget(params.history, docLimit);
    for (const p of docPaths) {
      const loaded = await loadUploadedDocument(p, docMaxChars);
      if (loaded) docCache.set(p, loaded);
    }
  }
  ...
  const messages = historyToModelMessages(
    ...
    docLimit,
    (relPath) => docCache.get(relPath) ?? null,
  );
...
export async function buildCurrentTurnContent(
  ...
  for (const msg of batch) {
    ...
    for (const file of files) {
      if (file.localPath) {
        ...
        parts.push({
          type: "text",
          text: `[Có file tài liệu đính kèm "${file.fileName}" nhưng không tải được về hệ thống]`,
        });
      }
    }
  }
  ...
  // Trong nhóm, nếu batch có tin từ nhiều người khác nhau, phải gắn tên
  // từng người vào từng dòng để LLM không hiểu nhầm ai nói gì.
  const uniqueSenders = isGroup
    ? new Set(batch.filter((m) => m.text.trim()).map((m) => m.senderName))
    : new Set<string>();
  ...
  const texts = batch
    ...
    .map((m) => {
      ...
      if (isGroup && multiSender) return `${m.senderName}: ${line}`;
      return line;
    });

  const label = isGroup && !multiSender ? `${latest.senderName}: ` : "";
  const body =
    texts.length > 0
      ? texts.join("\n")
      : parts.length > 0
        ? "(người dùng gửi file/ảnh đính kèm, không kèm chữ)"
        : "";
  if (body) {
