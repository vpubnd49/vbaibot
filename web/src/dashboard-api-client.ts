 ...
 */
export type LlmProviderKind = "openai-compatible" | "google" | "gemini";
...
export type OverviewData = {
  ...
  tokenSummaryByAccount?: { accountId: string; summary: TokenSummary }[];
  groupedUsageByAccount?: { accountId: string; items: GroupedUsageItem[] }[];
  system: {
...
export const api = {
  ...
  overview: (params?: { days?: number; granularity?: UsageGranularity } | number) => {
    if (typeof params === "number") {
      return request<OverviewData>(`/api/overview?days=${params}`);
    }
  ...
  knowledge: (accountId: string, status: string, q: string, page: number) =>
    request<{ items: SharedKnowledgeItem[]; hasMore: boolean; pendingCount: number }>(
      `/api/knowledge?accountId=${encodeURIComponent(accountId)}&status=${encodeURIComponent(status)}&q=${encodeURIComponent(q)}&page=${page}`,
    ),
  addKnowledge: (accountId: string, category: string, content: string, source: string) =>
    request<{ ok: boolean; id?: number; error?: string }>(`/api/knowledge`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ accountId, category, content, source }),
    }),
  approveKnowledge: (accountId: string, id: number) =>
  ...
  deleteKnowledge: (accountId: string, id: number) =>
    request<{ ok: boolean }>(`/api/knowledge/${id}?accountId=${encodeURIComponent(accountId)}`, {
      method: "DELETE",
    }),
  pendingKnowledgeCount: (accountId: string) =>
    request<{ count: number }>(`/api/knowledge/pending-count?accountId=${encodeURIComponent(accountId)}`),
