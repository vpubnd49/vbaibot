...
import { appendMessage, setMessageFiles, setMessageImages } from "../conversation/history-store.js";
...
function ghiVaoHistory(accountId: string, msg: ParsedMessage): void {
  ...
  if (msg.files && msg.files.length > 0) {
    void persistBatchFiles(accountId, [msg]).then(() => {
      const paths = filePathsOf(msg.files);
      if (paths) setMessageFiles(rowId, paths);
    });
  }
}
