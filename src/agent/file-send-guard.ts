...
const CAU_DA_GUI_FILE_RE =
  /(?:đã|vừa|em|sẽ|em\s+sẽ)\s+(?:gửi|xuất)\s+(?:anh|chị|bạn|thầy|cô|bác)?\s*(?:cho\s+(?:anh|chị|bạn|thầy|cô|bác))?\s*file\s+(?:excel|word|docx|xlsx|pdf|pptx|\b[a-zA-Z0-9_-]+\.(?:xlsx|docx|pdf|pptx)\b)/i;
const CAU_DANG_XUAT_CHO_RE =
  /(?:đang|đã|sẽ|em\s+(?:đang|đã|sẽ))\s+(?:xuất|gửi|thực\s+hiện(?:\s+lệnh|\s+quá\s+trình)?\s+(?:xuất|tạo|gửi))\s+file\s+.*?(?:cho|đến|tới)\s+(?:anh|chị|bạn)(?:\s+\p{Lu}\p{Ll}+)?/iu;
/**
...
const CAU_THUC_HIEN_XUAT_RE =
  /(?:đang|đã|sẽ|em\s+(?:đang|đã|sẽ|tiến\s+hành|thực\s+hiện))\s+(?:thực\s+hiện|tiến\s+hành)?(?:\s+(?:lệnh|quá\s+trình|việc))?\s+(?:xuất|tạo|gửi)\s+file/i;

const CAU_SE_XUAT_FILE_RE =
  /(?:em\s+)?(?:sẽ|đang|tiến\s+hành)\s+(?:xuất|tạo|gửi)\s+(?:một\s+)?file\s+(?:excel|word|docx|xlsx|pdf|pptx)/i;

/** Người dùng có ý định rõ ràng muốn xuất/chuyển sang file */
const Y_DINH_XUAT_FILE_RE =
  /(?:chuyển|xuất|tạo|lập|làm|làm\s+lại|lập\s+lại|chỉnh\s+sửa|soạn|viết|dựng|tiến\s+hành\s+(?:xuất|tạo|lập|làm))\s+(?:qua|sang|thành|ra)?\s*(?:file\s*)?(?:excel|word|powerpoint|pptx|xlsx|docx|pdf)/i;
const Y_DINH_XUAT_EXCEL_RE =
  /(?:xuất|chuyển|làm|làm\s+lại|lập|tạo|soạn)\s+(?:sang\s+|lại\s+)?(?:file\s+)?(?:excel|xlsx|bảng\s+tính)/i;
/** "tiến hành xuất", "xuất cho tôi", "gửi file cho tôi" — không kèm tên định dạng */
const Y_DINH_XUAT_CHUNG_RE =
  /(?:tiến\s+hành|hãy|vui\s+lòng|nhờ\s+bạn)\s+(?:xuất|tạo|gửi)(?:\s+file)?(?:\s+cho\s+(?:tôi|em|anh|chị))?/i;
