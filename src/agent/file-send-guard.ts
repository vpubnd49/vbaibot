...
const CAU_DA_GUI_FILE_RE =
  /(?:đã|vừa|em)\s+gửi\s+(?:anh|chị|bạn|thầy|cô|bác)?\s*(?:cho\s+anh|cho\s+chị)?\s*file\s+(?:excel|word|docx|xlsx|pdf|pptx|\b[a-zA-Z0-9_-]+\.(?:xlsx|docx|pdf|pptx)\b)/i;
const CAU_DANG_XUAT_CHO_RE =
  /(?:đang|đã)\s+(?:xuất|gửi)\s+file\s+.*?\s+(?:cho|đến|tới)\s+(?:anh|chị|bạn)/i;

/** Người dùng có ý định rõ ràng muốn xuất/chuyển sang file */
const Y_DINH_XUAT_FILE_RE =
  /(?:chuyển|xuất|tạo|lập)\s+(?:qua|sang|thành|ra)?\s*(?:file\s*)?(?:excel|word|powerpoint|pptx|xlsx|docx)/i;
const Y_DINH_XUAT_EXCEL_RE = /(?:xuất|chuyển)\s+(?:sang\s+)?excel/i;
const MO_TA_FILE_CUA_BOT_RE =
  /(?:em\s+đã\s+(?:thiết\s+lập|tạo|chuyển|chia|sắp\s+xếp)|file\s+excel\s+(?:gồm|được|với)|bảng\s+tính\s+gồm)/i;
...
export function laTinNhanAoGiacGuiFile(
  ...
  if (userPrompt) {
    const coYChuyenFile = Y_DINH_XUAT_FILE_RE.test(userPrompt) || Y_DINH_XUAT_EXCEL_RE.test(userPrompt);
