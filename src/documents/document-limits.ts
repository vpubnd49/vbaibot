...
export function safeFileName(
  raw: string,
  extension: "docx" | "xlsx" | "md" | "txt" | "csv" | "html" | "pdf" | string,
): string {
  const base = raw
...
export function checkPresentationLimits(slides: PptxSlide[]): LimitCheck {
  const maxSlides = getTuning("DOCUMENT_MAX_SLIDES");
  if (slides.length > maxSlides) {
    return fail(
      `Bài trình chiếu có ${slides.length} slide, vượt trần ${maxSlides}. Hãy rút gọn rồi gọi lại.`,
    );
  }
  return ok;
}
