...
export async function runAgentTurn({
  ...
  // + tri thức dùng chung đã được admin duyệt
  const sharedKnowledge = getApprovedKnowledge(account.id);
  const memory = isolated
    ? { facts: [], threadSummary: "", sharedKnowledge }
    : {
  ...
  // Auto-capture: lưu VĨNH VIỄN kết quả tra cứu có giá trị vào DB.
  // Fire-and-forget: chạy đồng bộ nhưng bọc try/catch bên trong,
  // KHÔNG BAO GIỜ làm hỏng câu trả lời gửi cho người dùng.
  if (!isolated) {
